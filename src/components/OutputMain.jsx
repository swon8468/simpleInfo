...
function OutputMain() {
  ...
  const RoadmapDisplay = ({ controlData }) => {
    ...
    // 제어 데이터가 변경될 때마다 이미지 업데이트
    useEffect(() => {
      if (controlData && controlData.currentPage === 'roadmap' && controlData.selectedImageId) {
        console.log('출력용: 제어 데이터에서 교내 배치 이미지 선택:', controlData);
        
        // campusImages에서 해당 이미지 찾기
        const image = campusImages.find(img => img.id === controlData.selectedImageId);
        if (image) {
          console.log('출력용: 이미지 찾음, 상태 업데이트:', image);
          setSelectedImage(image);
        } else if (controlData.imageURL) {
          console.log('출력용: 직접 전송된 이미지 데이터 사용:', controlData);
          setSelectedImage({
            id: controlData.selectedImageId,
            imageURL: controlData.imageURL,
            buildingName: controlData.buildingName,
            floorNumber: controlData.floorNumber,
            description: controlData.description
          });
        }
      }
    }, [controlData, campusImages]);
