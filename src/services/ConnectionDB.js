...
class ConnectionDB {
  ...
  // 제어 데이터 리스너 (출력용 디바이스용)
  listenToControlData(sessionId, callback) {
    const docRef = doc(db, 'connections', sessionId);
    
    const unsubscribe = onSnapshot(docRef, (doc) => {
      if (doc.exists()) {
        const data = doc.data();
        console.log('ConnectionDB: 제어 데이터 변경 감지:', sessionId, data);
        
        // controlData가 있는 경우 콜백 호출
        if (data.controlData) {
          callback(data.controlData);
        }
      } else {
        console.log('ConnectionDB: 출력용 세션이 존재하지 않음:', sessionId);
      }
